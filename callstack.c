...
struct callstack_s *callstack_current(void)
{
	...
	/* search */
	struct hlist_node *p = hash_search(g_callstack_hash, ips, sizeof(unw_word_t) * ip_num);
	if(p != NULL) { /* found */
...
static int callstack_cmp(const void *a, const void *b)
{
	...
	return (csa->expired_count - csa->free_expired_count) - (csb->expired_count - csb->free_expired_count);
}
