...
void memblock_delete(struct memblock_s *mb)
{
	...
	if(free_time > cs->free_max)
		cs->free_max = free_time;
	if(cs->free_min == 0 || free_time < cs->free_min)
		cs->free_min = free_time;
...
{
	struct hlist_node *p = hash_search(g_memblock_hash, &pointer, sizeof(uintptr_t));
	if(p == NULL) {
		return NULL;
	}
